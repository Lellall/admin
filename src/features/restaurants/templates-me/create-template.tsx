...
import { useCreateTemplateMutation } from "@/redux/templates";
    ...
    <div>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        ...
        <div className="mt-10">
          ...
          <SelectedProductsList>
            ...
            {selectedProducts.map((product) => (
              <ProductItem key={product.value}>
                <span>
                  {product.label} - {formatCurrency(product.price)}
                </span>
                <div className="flex m-2 w-[30%] items-center justify-around">
                  <QuantityControl>
                    <Button onClick={() => handleQuantityChange(product.value, -1)}>
                      -
                    </Button>
                  ...
                  <RemoveButton onClick={() => handleRemoveProduct(product.value)}>
                    Remove
                  </RemoveButton>
                </div>
              </ProductItem>
          ...
          <SubmitButton disabled={isLoading}>{buttonTitle}</SubmitButton>
        </div>
      </form>
    </div>
  );
