...
function SearchComponent({ setSelectedProducts }: { setSelectedProducts: (p: any) => void }) {
  ...
  const fetchOptions = async (inputValue: string, callback: (item: any) => void) => {
    ...
    try {
      const response = await fetch(`${configUrl.BACKEND_URL}/products?page=0&size=10&filter=${inputValue}`)
