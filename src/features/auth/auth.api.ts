...
import { setAuthState, logout } from '../auth/authSlice';
    ...
    endpoints: (builder) => ({
        login: builder.mutation({
            ...
            async onQueryStarted(arg, { dispatch, queryFulfilled }) {
                try {
                    ...
                    dispatch(setAuthState({ isAuthenticated: true, accessToken: data.access_token, refreshToken: data.refresh_token }));
                } catch (err) {
                    console.error(err);
                }
            },
        ...
        refreshToken: builder.mutation({
            ...
            async onQueryStarted(arg, { dispatch, queryFulfilled }) {
                try {
                    const { data } = await queryFulfilled;
                    dispatch(setAuthState({ isAuthenticated: true, accessToken: data.access_token, refreshToken: localStorage.getItem('refresh_token') }));
                } catch (err) {
                    console.error(err);
                    dispatch(logout());
                }
